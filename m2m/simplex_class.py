...
class Element(object):
    def __init__(self, label=None, elt_type=None,
        ...
        if nodes != None: 
            self.nodes = np.array(nodes,dtype=int)  #node index
        elif l_nodes!=None and map_nodes==None:     #node labels -> node index
            self.nodes = np.array([x-1 for x in l_nodes],dtype=int)
        elif l_nodes!=None and map_nodes!=None:     #irrugular node labels 
            self.nodes = np.array([bisect.bisect_left(map_nodes,x) for x in l_nodes],
                                  dtype=int)
